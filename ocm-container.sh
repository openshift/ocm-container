...
### MANAGED-NOTIFICATIONS-DIR File Mounting
if [ -d "${MANAGED_NOTIFICATIONS_DIR}" ]; then
  MANAGED_NOTIFICATIONS_DIR_RW_FLAG="ro"
  if [[ ${MANAGED_NOTIFICATIONS_DIR_RW} = true ]]; then
    MANAGED_NOTIFICATIONS_DIR_RW_FLAG="rw"
  fi
  MANAGED_NOTIFICATIONS_DIR_MOUNT="
-v ${MANAGED_NOTIFICATIONS_DIR}:/root/managed-notifications:${MANAGED_NOTIFICATIONS_DIR_RW_FLAG}
"
fi
