...
func New(cmd *cobra.Command, args []string) (*ocmContainer, error) {
	...
	// OCM SDK Client
	client, err := ocm.NewClient(ocmConfig.Env["OFFLINE_ACCESS_TOKEN"], ocmConfig.Env["OCM_URL"])
	if err != nil {
		return o, err
	}
	defer client.Close()
	...
	if key != "" {
		o.NonBlockingPostStartExecCmds = append(o.NonBlockingPostStartExecCmds, []string{"ocm", "login", "--token", ocmConfig.Env["OFFLINE_ACCESS_TOKEN"]})
		o.NonBlockingPostStartExecCmds = append(o.NonBlockingPostStartExecCmds, []string{"ocm", "backplane", "login", key})
		if !isValidClusterKey(key) {
			return o, errInvalidClusterId
		}

		ocmV1Cluster, err := getCluster(client, key)
		if err != nil {
			return o, err
		}
		...
		if ocmV1Cluster.Hypershift().Enabled() {
			hcp, err = GetHcp(client, ocmV1Cluster)
			if err != nil {
				return o, err
			}
			log.Debug("retrieved cluster HCP info from OCM")
		} else {
			hive, err = GetHive(client, ocmV1Cluster)
			if err != nil {
				return o, err
			}
