...
# Show quota info for the user, authenticated or not
# Pipes to true because unauthenticated requests will always have 
# a zero quota for the GraphQL API
RUN github_dl quota || true 
...
RUN /bin/bash -c "PATH=${PATH}:${BACKPLANE_BIN_DIR}/latest /usr/local/bin/backplane-tools install ocm backplane-cli oc"
...
# Download the checksum and binary, and validate them
RUN github_dl download --url ${OMC_URL} --checksum_file checksums.txt --checksum_algorithm ${OMC_CHECKSUM_ALGORITHM} --platform ${OMC_PLATFORM_PREFIX}$(platform_convert "@@PLATFORM@@" --amd64 --arm64)${OMC_PLATFORM_SUFFIX}
...
# Download the checksum and binary, and validate them
RUN github_dl download --url ${JIRA_URL} --checksum_file checksums.txt --checksum_algorithm ${JIRA_CHECKSUM_ALGORITHM} --platform ${JIRA_PLATFORM_PREFIX}$(platform_convert "@@PLATFORM@@" --amd64 --arm64)
...
ENV K9S_CHECKSUM_FILE="sha256sum.txt"
ENV K9S_CHECKSUM_ALGORITHM="sha256"
ENV K9S_PLATFORM_PREFIX="Linux_"
ENV K9S_PLATFORM_SUFFIX=".tar.gz"
...
# Download the checksum and binary, and validate them
RUN github_dl download --url ${K9S_URL} --checksum_file ${K9S_CHECKSUM_FILE} --checksum_algorithm ${K9S_CHECKSUM_ALGORITHM} --platform ${K9S_PLATFORM_PREFIX}$(platform_convert "@@PLATFORM@@" --amd64 --arm64)${K9S_PLATFORM_SUFFIX}
...
ENV NODEPP_CHECKSUM_FILE="sha256sum.txt"
